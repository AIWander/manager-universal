...
enum TaskStatus {
    ...
    Stalled,
    /// v1.4: Task has exhausted its retry budget (retry_count >= max_retries).
    FailedMaxRetries,
}
...
impl std::fmt::Display for TaskStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TaskStatus::FailedMaxRetries => write!(f, "failed_max_retries"),
        }
    }
}
...
/// v1.4: Heuristic for default stall_recovery value based on prompt content.
...
struct Task {
    ...
    /// v1.4: Controls stall recovery -- auto retries on dead PID, manual requires resume_task.
    #[serde(default)]
    pub stall_recovery: StallRecovery,
    /// v1.4: Distinguishes why the task entered Paused status.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub paused_reason: Option<PausedReason>,
    /// v1.4: Forward link from original task to its retry successor.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Server {
    ...
    /// v1.4 dual-source: merges entries from OPS and LOCAL breadcrumb dirs.
    /// Deduplicates by breadcrumb id (last-seen wins). Missing dirs = empty source, no error.
    /// Env vars: OPS_BREADCRUMBS_DIR, LOCAL_BREADCRUMBS_DIR
    fn read_breadcrumb_state() -> Option<String> {
        ...
        fn collect_entries(dir: &str, map: &mut std::collections::HashMap<String, Value>) {
            let jsonl_path = format!(r"{}\breadcrumb.jsonl", dir);
        ...
        // Read ops first, then local (local wins on conflict)
        collect_entries(&ops_dir, &mut merged);
        collect_entries(&local_dir, &mut merged);

        if merged.is_empty() {
            return None;
        }

        // Find the most recent active (non-complete) breadcrumb
        let active = merged.values().find(|v| {
            v.get("status")
                .and_then(|s| s.as_str())
                .map(|s| s != "complete" && s != "aborted")
                .unwrap_or(false)
        });
        ...
        if let Some(v) = active {
            ...
            return Some(format!(
                "[CONTEXT: Active breadcrumb: {} (step {}/{})]",
                name, step, total
            ));
        }
        None
    ...
    fn prepare_retry(task: &mut Task) -> Option<Task> {
        ...
        if task.retry_count >= task.max_retries {
            task.status = TaskStatus::FailedMaxRetries;
            return None;
        }
        ...
        let new_task = Task {
            ...
            notify_title: None,
            notify_body: None,
            stall_recovery: task.stall_recovery.clone(),
            paused_reason: None,
            retried_as: None,
        };

        // Note on original task and set forward link
        task.output
            .push_str(&format!("\n[Retrying as {}]", new_task_id));
        task.retried_as = Some(new_task_id.clone());
        info!(
...
/// v1.4: Backoff before launching the next retry attempt.
...
fn spawn_retry_execution(
    ...
    let backoff = retry_backoff_secs(retry_task.retry_count);
    ...
    match retry_task.backend {
        Backend::Gpt => {
            if let Some(cfg) = config {
                handle.spawn(async move {
                    ...
                    run_gpt_task(cfg, tasks, tid).await;
                });
            } else {
        ...
        Backend::ClaudeCode => {
            ...
            let cmd = claude_code_cmd().to_string();
            handle.spawn(async move {
                if backoff > 0 {
                    tokio::time::sleep(std::time::Duration::from_secs(backoff)).await;
                }
                run_cli_task(tasks, tid, &cmd, args, None, StdinMode::Null).await;
            });
        }
        Backend::Codex => {
            let args = vec![
            ...
            handle.spawn(async move {
                ...
                run_codex_task(tasks, tid, args, wd).await;
            });
        }
        Backend::Gemini => {
            let mut args = vec![
            ...
            let node = node_cmd().to_string();
            handle.spawn(async move {
                if backoff > 0 {
                    tokio::time::sleep(std::time::Duration::from_secs(backoff)).await;
                }
                run_cli_task(tasks, tid, &node, args, None, StdinMode::Null).await;
            });
        }
    }
}
...
fn spawn_on_complete(
    ...
    let follow_up = Task {
        ...
        notify_title: None,
        notify_body: None,
        stall_recovery: StallRecovery::Auto,
        paused_reason: None,
        retried_as: None,
    };
...
fn handle_submit_task(server: &Server, params: Value) -> Result<Value, String> {
    ...
    // v1.4: Read max_retries and stall_recovery from caller; fall back to smart defaults.
    let max_retries = params
        .get("max_retries")
        .and_then(|v| v.as_u64())
        .map(|v| v as u32)
        .unwrap_or_else(default_max_retries);
    let stall_recovery = match params.get("stall_recovery").and_then(|v| v.as_str()) {
        Some("manual") => StallRecovery::Manual,
        Some("auto") => StallRecovery::Auto,
        _ => {
            if is_destructive_prompt(&raw_prompt) {
...
fn handle_get_status(server: &Server, params: Value) -> Result<Value, String> {
    ...
    let health = match task.status {
        ...
        TaskStatus::FailedMaxRetries => "failed",
...
/// v1.4: Stall watchdog -- detects genuinely dead agent processes via PID liveness check.
/// Rather than using time-based output silence (which produces false positives on slow tools
/// like cargo builds or model inference), we check whether the child process is still alive.
/// Tasks without a child_pid are skipped -- they are in-process or unspawned and cannot
/// be liveness-checked. The `_timeout_secs` parameter is retained for call-site compatibility
/// but is not read inside the function.
async fn stall_watchdog(tasks: Arc<RwLock<HashMap<String, Task>>>, _timeout_secs: u64) {
    let mut interval = tokio::time::interval(std::time::Duration::from_secs(5));
    interval.tick().await; // skip first tick -- let tasks settle
    loop {
        interval.tick().await;

        // Read pass: collect Running tasks whose child process is confirmed dead.
        // Tasks with no child_pid are skipped (cannot check liveness).
        let to_act: Vec<(String, String, StallRecovery)> = {
            let store = tasks.read().await;
            store
                .values()
                .filter(|t| t.status == TaskStatus::Running)
                .filter_map(|t| {
                    let pid = t.child_pid?; // None -> skip, no PID to check
                    if is_process_alive(pid) {
                        None // process alive -> leave it alone
                    } else {
                        let label = t.label.clone().unwrap_or_else(|| t.id.clone());
                        Some((t.id.clone(), label, t.stall_recovery.clone()))
                    }
                })
                .collect()
        };
        ...
        // Write pass: mark Failed or Paused depending on stall_recovery (re-check inside lock).
        {
            let mut store = tasks.write().await;
            for (task_id, _, recovery) in &to_act {
                if let Some(task) = store.get_mut(task_id) {
                    if task.status == TaskStatus::Running {
                        match recovery {
                            StallRecovery::Auto => {
                                task.status = TaskStatus::Failed;
                                task.error = Some("orphaned-pid-dead".into());
                                Server::persist_task(task);
                            }
                            StallRecovery::Manual => {
                                task.status = TaskStatus::Paused;
                                task.paused_reason = Some(PausedReason::StallManualRecovery);
                                Server::persist_task(task);
                            }
                        }
                    }
        ...
        // Fire toasts outside the lock -- one per newly-detected dead task.
        for (_, label, recovery) in to_act {
            let (title, body) = match recovery {
                StallRecovery::Auto => (
                    format!("[DEAD] {}", label),
                    "Child process exited. Queuing retry.".to_string(),
                ),
                StallRecovery::Manual => (
                    format!("[DEAD] {}", label),
